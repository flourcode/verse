# Better Verses: development notes
...
Comfort-first ranking: when the query sounds like a hard moment (a hand-picked page matched, or words like grieving, scared, dying, estranged, my mom…), verses that comfort rank up (psalms, gospels, letters, verses spoken to someone, words like near, heal, refuge, with you), and law codes, genealogies, and verses about punishment or sexual sin rank down. Partial matches rank lower. Nothing is removed; the results say so and offer "Rank by words alone," and the Rank filter switches it either way. A plain word like `clay` or a reference never triggers it.
...
Mobile first, ~544px interactive width, ~704px reading width, Inter 17px for the interface and Source Serif for Scripture, both self-hosted variable woff2 files with `font-display: swap`, verse text 22–28px with a fluid clamp. Palette: one green hue in five tones on a white page (Material green 50 `#E8F5E9`, 200 `#A5D6A7`, 400 `#66BB6A`, 800 `#2E7D32`, 900 `#1B5E20`). 50 holds the search box and Today's verse; 200 is selected chips and matched words; 400 is hope, rationed to a moment or two and never text; 800 is the button, links, and references; 600 (`#43A047`) is the B tile and the copied-verse toast; 900 appears only as a button's pressed state. Roughly 70% white. Scripture is always near-black; the app is green, the Word is the content. Auto dark mode with a tonal equivalent. Tokens are named as roles (surface / on-surface / primary / outline) and every text/background pairing clears WCAG AA 4.5:1 in both themes. 48px touch targets, visible focus, reduced-motion respected. Words of Jesus in red (toggle `redLetter` in the config). Automatic dark mode via `prefers-color-scheme` with a `[data-theme]` override hook. Tap targets 44px. `prefers-reduced-motion` disables the one animation. No images on content pages, no third-party scripts unless you turn them on.
...
**Every Monday morning**, the GitHub Action in `.github/workflows/newsletter.yml` generates the week's issue and saves it in the repo under `newsletter/<date>/`, ready for a **Tuesday 8:00 AM** send (schedule it in Kit; Tuesday tends to beat Monday's crowded inbox, and the site copy names no day, so you can move it later on your own open rates). Nothing is sent from the repo. Open that folder on GitHub, copy `email.html` into a Kit broadcast (HTML editor), paste the subject from `subject.txt`, and send. You can also run the Action any time from the repo's Actions tab, optionally with an issue number to preview ahead.
...
To show the signup on the site, paste the `action` URL from your Kit form's HTML embed into `site.config.js` → `newsletter.formAction` (it looks like `https://app.kit.com/forms/1234567/subscriptions`). The site then renders its own one-field form that posts straight to Kit: no Kit script, no redirect away, and it matches the site. In the Kit form's settings, set "after subscribing" to redirect to `https://betterverses.com/thanks/`. The signup reads "Something worth reading this week" with the button "Send me Better Verses." The full block appears on the home page after Today's verse, on the Today page, on the verse-in-context pages, and at the end of every journal post; a compact form sits in the footer of every page. It never appears beside a verse on a situation page. If you'd rather link out to a Kit landing page instead, set `newsletter.url` and leave `formAction` empty. With neither set, nothing renders.
