...
(function () {
  ...
  function initToday() {
    ...
    const d = JSON.parse(holder.textContent); // { pool: [{id,ref,text,context,question}], epochDay }
    const day = Math.floor(Date.now() / 86400000); // UTC day number; same verse for everyone for a calendar day (UTC)
    const i = ((day % d.pool.length) + d.pool.length) % d.pool.length;
    const entry = d.pool[i];
  ...
  document.addEventListener('DOMContentLoaded', () => { initFinder(); initHomeDaily(); initTopic(); initToday(); initRandom(); });
})();
